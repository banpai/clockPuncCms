            ...
            <Card>
                ...
                <Row :gutter="10">
                    ...
                    <Form class="step-form" ref="step" :model="step" :rules="stepRules" :label-width="100">
                        ...
                        <FormItem label="鸡汤名言1：" prop="wellknow1">
                            <Input v-model="step.wellknow1" type="text" :autosize="{minRows: 2,maxRows: 5}" placeholder="25个字内" />
                        </FormItem>
                        <FormItem label="鸡汤名言2：" prop="wellknow2">
                            <Input v-model="step.wellknow2" type="text" :autosize="{minRows: 2,maxRows: 5}" placeholder="25个字内" />
                        </FormItem>
                        <FormItem label="">
    ...
    export default {
        ...
        methods: {
            init() {
                var params = {};
                // 初始化数据
                util.ajax.post('/api/clockPunch/cms/getData', params).then(response => {
                    console.log(response.data);
                    this.img1 = response.data.host + response.data.image;
                    this.positioan = response.data.position;
                    this.size = response.data.size;
                    this.step.wellknow1 = response.data.wellKnow1;
                    this.step.wellknow2 = response.data.wellKnow2;
            ...
            handleSubmit() {
              ...
              this.$refs['step'].validate((valid) => {
                if(valid){
                  if(this.step.wellknow1.length > 25){
                    this.$Message.warning('鸡汤名言限定25个字内！');
                  }else if (this.step.wellknow2.length > 25){
                    this.$Message.warning('鸡汤名言限定25个字内！');
                  }else{
                    var vm = this;
                    let base64 = false;
                    ...
                    let param = {
                        ...
                        size: vm.size,
                        wellKnow1: vm.step.wellknow1,
                        wellKnow2: vm.step.wellknow2
                    };
                    //添加请求头
