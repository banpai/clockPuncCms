    ...
    methods: {
        ...
        saveEditPass () {
            ...
            this.$refs['editPasswordForm'].validate((valid) => {
                if (valid) {
                    ...
                    var params = {
                        user: user,
                        password: that.editPasswordForm.oldPass,
                        newPassword: that.editPasswordForm.rePass
                    };
                    util.ajax.post('/api/clockPunch/cms/changePassword', params).then(m=>{
                        // console.log(m.data);
                        Cookies.set('password', that.editPasswordForm.rePass);
                        this.$Message.success({
                            content: '修改成功！',
                            duration: 2,
                            closable: false,
                            onClose: function(){
                                that.editPasswordForm.oldPass = '';
                                that.editPasswordForm.newPass = '';
                                that.editPasswordForm.rePass = '';
                                that.cancelEditPass();
                            }
                        });
                        this.savePassLoading = false;
        ...
        saveInfoAjax () {
            ...
            var params = {
                user: user,
                password: password,
                newUser: that.userForm.name
            };
            util.ajax.post('/api/clockPunch/cms/changeName', params).then(m=>{
                // console.log(m.data);
                Cookies.set('user', that.userForm.name);
